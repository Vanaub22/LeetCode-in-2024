...
public:
    int maxDistance(vector<vector<int>>& arrays) {
        ...
        for(int i=0;i<m;i++) {
            ...
            if(a<min1.second) min2=min1,min1={i,a};
            else if(a<min2.second && a>=min1.second) min2={i,a};
            if(b>max1.second) max2=max1,max1={i,b};
            else if(b>max2.second && b<=max1.second) max2={i,b};
        }
